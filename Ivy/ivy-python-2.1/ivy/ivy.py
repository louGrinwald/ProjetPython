...
def UDP_init_and_listen(broadcast_addr, port, socket_server):
    ...
    s.sendto(("%li %s %s %s\n"%(PROTOCOL_VERSION,socket_server.port,
                               socket_server.agent_id, socket_server.agent_name)).encode ('ascii'),
             (broadcast_addr, port))
...
class IvyClient:
    ...
    def _send(self, msg_type, *params):
        ...
        import socket
        try:
            self.socket.send(encode_message(msg_type, *params).encode('ascii'))
...
class IvyHandler(socketserver.StreamRequestHandler): #BaseRequestHandler):
    ...
    def handle(self):
        ...
        socket.send(encode_message(START_INIT, self.server.port,
                                   self.server.agent_name).encode('ascii'))
        for idx, subscr in self.server.get_subscriptions():
            socket.send(encode_message(ADD_REGEXP, idx, subscr).encode('ascii'))
        socket.send(encode_message(END_REGEXP, 0).encode('ascii'))
        
        while self.server.isAlive():
            from socket import error as socket_error
            from socket import timeout as socket_timeout
            try:
                msgs = socket.recv(bufsize).decode ('ascii')
                #benoit
                print ("reception : " + msgs)
            ...
            if msgs[-1:] != '\n':
                ...
                while _msg[-1:] != '\n' and self.server.isAlive():
                    ...
                    while self.server.isAlive():
                        try:
                            _msg = socket.recv(bufsize).decode('ascii')
                            #benoit
                            print ("while " + _msg)
                            break
                        except socket_timeout:
