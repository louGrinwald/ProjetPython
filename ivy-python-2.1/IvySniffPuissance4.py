...
#La fontion qui gere ce qui se passe quand on joue sur une colonne
def on_colonne(agent, *arg):
    colonne = int(arg[0])
    if(colonne < 8):
...
if __name__ == '__main__':
    ...
    colonne:1-7 pour jouer sur la colonne 1 a 7
    victoire pour signaler la victoire du joueur
    '''
    IvyBindMsg(on_colonne, '^colonne:([1-7])$')
    IvyBindMsg(on_victoire, '^victoire$')
